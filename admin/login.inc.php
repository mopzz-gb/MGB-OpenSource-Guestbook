	...
	*/
	...
	if(isset($_GET['action']) AND ($_GET['action'] == "logout")) {
		...
		if(isset($_SESSION['user_ID'])) {
			mgb_sql_connect($mysqli, "UPDATE ".$db['prefix']."user SET `logged_out` = '1' WHERE ID=".secure_value($_SESSION['user_ID'])." LIMIT 1", "Error while logging out.", 0);
			...
			if(!empty($_POST['username']) AND !empty($_POST['password'])) {
				if(login_ok($mysqli, $_POST['username'], "", $_POST['password'])) {
					if(!isset($_SESSION['user_key'])) {
						generate_key_and_pw(MGB_ROOT, $mysqli, secure_value($_POST['username']), 16, "adminpanel");

						$result = mgb_sql_connect($mysqli, "SELECT ID, user_name, user_key, user_ip, user_is_active, logged_out FROM ".$db['prefix']."user WHERE user_name=".secure_value($_POST['username']), "Error while logging in.", 1);
						...
						if($_SESSION['user_is_active'] == 1) {
							...
							mgb_sql_connect($mysqli, "UPDATE ".$db['prefix']."user SET `logged_in` = '".time()."', `logged_out` = '0' WHERE ID=".secure_value($user['ID'])." LIMIT 1", "Error while logging in.", 0);
						} else {
							session_unset();
							session_destroy();
							$_SESSION = array();
