		...
		if(check_rights($mysqli, $_GET['action'], $_SESSION['user_ID'])) {
			...
			if(!empty($_POST['dropbox'])) {
				if($_POST['dropbox'] == 1) { // Delete all spam entries
					mgb_sql_connect($mysqli, "TRUNCATE ".$db['prefix']."banlist_ips", "Error while deleting all ip entries.", 0, null, null);
					...
					*/
					...
					$result = mgb_sql_connect($mysqli, "SELECT banned_ip, banned_ip_first, banned_ip_second, banned_ip_third, banned_ip_fourth, timestamp FROM ".$db['prefix']."banlist_ips ORDER BY timestamp ASC", "Error while loading data from banlist_ips for sql export.", 1, null, null);
			...
			if(isset($_GET['id'])) {
				if(isset($_GET['spam_action'])) {
					if($_GET['spam_action'] == 'delete') {
						$sql = "DELETE FROM `".$db['prefix']."banlist_ips` WHERE ID = ? LIMIT 1";
						$params = [$_GET['id']];
						$types = "i";
						mgb_sql_connect($mysqli, $sql, "Error while deleting a single ip entry.", 0, $params, $types);
					}
				}
			}
			...
			$results = mgb_sql_connect($mysqli, $sql, "Error while counting banned ip entries.", 1, null, null);
			...
			if ($_GET['p'] == 1) {
				$sf_forwards = "<a class=\"admin\" href=\"admin.php?action=banlist_ips&amp;orderby=".$_GET['orderby']."&amp;sort=".$_GET['sort']."&amp;p=".($_GET['p'] + 1)."\" title=\"".$lang['page_forwards']."\">".$lang['page_forwards_symbol']."</a>";
			...
			if ($_GET['p'] > 1) {
				if (($pages_total >= 3) AND ($_GET['p'] > 2)) {
					$sf_first = "<a class=\"admin\" href=\"admin.php?action=banlist_ips&amp;orderby=".$_GET['orderby']."&amp;sort=".$_GET['sort']."&amp;p=1"."\" title=\"".$lang['page_first']."\">".$lang['page_first_symbol']."</a>";
				}
				$sf_backwards = "<a class=\"admin\" href=\"admin.php?action=banlist_ips&amp;orderby=".$_GET['orderby']."&amp;sort=".$_GET['sort']."&amp;p=".($_GET['p'] - 1)."\" title=\"".$lang['page_backwards']."\">".$lang['page_backwards_symbol']."</a>";
				$sf_pagenumber = $_GET['p'];
				$sf_forwards = "<a class=\"admin\" href=\"admin.php?action=banlist_ips&amp;orderby=".$_GET['orderby']."&amp;sort=".$_GET['sort']."&amp;p=".($_GET['p'] + 1)."\" title=\"".$lang['page_forwards']."\">".$lang['page_forwards_symbol']."</a>";
			...
			$allowed_columns = ['id', 'banned_ip', 'matches', 'timestamp'];
			...
			$sql = "SELECT id, banned_ip, matches, timestamp FROM ".$db['prefix']."banlist_ips ORDER BY $orderby $sort LIMIT $load_start, $load_end";			
			...
			if(!empty($entry)) {
				for($i = 0; $i < count($entry); $i++) {
					...
					$page_entry[$i] = mgb_template_replace([
						'ENTRY_ID' 			=> $entry[$i]['id'],
						'ENTRY_IP' 			=> $entry[$i]['banned_ip'],
						'ENTRY_EMAIL' 		=> "",
						'ENTRY_DOMAIN' 		=> "",
						'ENTRY_MATCHES' 	=> $entry[$i]['matches'],
						'ENTRY_TIMESTAMP' 	=> $entry_timestamp,
						'DELETE' 			=> "<a href=\"admin.php?action=banlist_ips&amp;id=".$entry[$i]['id']."&amp;spam_action=delete".$add_page_nr."\" onClick=\"return confirm('".$entry[$i]['id'].", ".$entry[$i]['banned_ip'].":&nbsp;{LANG_CONFIRM_DELETE}'); submit();\"><img class=\"icon\" src=\"templates/default/images/delete.png\" title=\"".$lang['delete_entry']."\" alt=\"".$lang['delete_entry']."\"></a>"
					], $page_entry[$i]);
