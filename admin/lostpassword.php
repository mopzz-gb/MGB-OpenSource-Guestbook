	...
	*/
	...
	if(isset($_GET['id']) AND isset($_GET['key'])) {
		...
		if($_GET['key'] === $user['np_key'] AND $user['np_expiration'] > time()) {
			...
			$lang['sendmail_new_password_created_title'] = format_mail(xhtmlbr2nl($lang['sendmail_new_password_created_title']), $user_name, "", time(), "", $settings['h_domain'], "", "", "", "", "", "", $new_password);
			$lang['sendmail_new_password_created_text'] = format_mail(xhtmlbr2nl($lang['sendmail_new_password_created_text']), $user_name, "", time(), "", $settings['h_domain'], "", "", "", "", "", "", $new_password);
		...
		if(!isset($_POST['sent'])) {
			...
			if(isset($_POST['email']) AND mgb_check_mail($_POST['email'])) {
				...
				if($lostpassword['np_expiration'] <= time()) {
					...
					$lang['sendmail_new_password_title'] = format_mail(xhtmlbr2nl($lang['sendmail_new_password_title']), $user_name, "", time(), "", $settings['h_domain'], $url_to_gb, "", "", "", $new_password_key, $user_id, "");
					$lang['sendmail_new_password_text'] = format_mail(xhtmlbr2nl($lang['sendmail_new_password_text']), $user_name, "", time(), "", $settings['h_domain'], $url_to_gb, "", "", "", $new_password_key, $user_id, "");
