<?php
	...
	*/
	...
	if (!defined('ADMINISTRATION')) {
		...
		// load template
		$content_settings_usage = mgb_load_template("admin", "default", "settings_telemetry", $settings['debug_mode']);

		if(!isset($_GET['action'])) { $_GET['action'] = "settings_telemetry"; }
		if(check_rights($mysqli, $_GET['action'], $_SESSION['user_ID'])) {
			if(isset($_POST['sent_settings']) AND $_POST['sent_settings'] == 1) {
				...
				if(empty($_POST['telemetry_ping'])) {
					$_POST['telemetry_ping'] = "https://ping.m-gb.org/ping.php";
				}
			...
			$selected_telemetry_1 = "";

			// value replacement
			if ($settings['telemetry'] == 0) { $selected_telemetry_0 = " selected"; } else { $selected_telemetry_1 = " selected"; }
			$page_include = mgb_template_replace([
				'SELECTED_TELEMETRY_0'  	=> $selected_telemetry_0,
				'SELECTED_TELEMETRY_1'  	=> $selected_telemetry_1,
				'EDIT_TELEMETRY_PING' 		=> $settings['telemetry_ping'],
				'EDIT_TELEMETRY_INSTALL_ID'	=> $settings['telemetry_install_id']
			], $page_include);
