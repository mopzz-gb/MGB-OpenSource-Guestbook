<?php
	...
	*/
	...
	$sqldescription[13] = "- Inserting values for 'search engines' in settings table...";
	$sqlisinsert[13] = 1;
	...
	$sql[37] = "ALTER TABLE `".$db['prefix']."settings` ADD `telemetry` TINYINT(1) DEFAULT NULL AFTER `debug_mode`;";
	$sqldescription[37] = "Adding telemetry...";
	$sql[38] = "ALTER TABLE `".$db['prefix']."settings` ADD `telemetry_ping` VARCHAR(255) NOT NULL DEFAULT 'https://ping.m-gb.org/ping.php' AFTER `telemetry`;";
	$sqldescription[38] = "Adding telemetry ping address...";
	$sql[39] = "ALTER TABLE `".$db['prefix']."settings` ADD `telemetry_install_id` CHAR(128) AFTER `telemetry_ping`;";
	$sqldescription[39] = "Adding telemetry unique install id...";
	$sql[40] = "ALTER TABLE `".$db['prefix']."settings` ADD `telemetry_last_ping` INT(11) AFTER `telemetry_install_id`;";
	$sqldescription[40] = "Adding telemetry last ping...";
	...
	$sqlisinsert[41] = 1;
	$sql[41] = "UPDATE `".$db['prefix']."settings` SET `telemetry_install_id` = '".$install_id."'";	
	$sqldescription[41] = "Adding unique install id...";
	
	// update banlists
	$sql[42] = "ALTER TABLE `".$db['prefix']."banlist_emails` DROP `banned_email_first`, DROP `banned_email_second`;";
	$sqldescription[42] = "Updating structure of email banlist...";
	$sql[43] = "ALTER TABLE `".$db['prefix']."banlist_ips` DROP `banned_ip_first`, DROP `banned_ip_second`, DROP `banned_ip_third`, DROP `banned_ip_fourth`;";
	$sqldescription[43] = "Updating structure of ip banlist...";
	
	if(isset($_POST['update_version']) AND $_POST['update_version'] == 1) {
		$sql[44] = "UPDATE `".$db['prefix']."settings` SET `version` = '".MGB_VERSION."'";
		$sqldescription[44] = "- Updating version number...";
	}
?>
