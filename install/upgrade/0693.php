<?php
	...
	*/
	...
	$sqldescription[27] = "- Adding 'fb' to entries table...";
	...
	$sql[30] = "UPDATE `".$db['prefix']."settings`
				SET `dateform` = 'd.m.Y, H:i';";
	...
	$sql[34] = "ALTER TABLE `".$db['prefix']."settings`
				ADD `captcha_salt` VARCHAR( 255 ) NOT NULL DEFAULT '".mt_rand()."' AFTER `captcha_max_length`";
	...
	$sql[40] = "ALTER TABLE `".$db['prefix']."spam_log`
			ADD `hp` VARCHAR( 255 ) NOT NULL AFTER `user_agent`";
	...
	$sql[47] = "ALTER TABLE `".$db['prefix']."settings`
			ADD `direct_access_text` VARCHAR( 255 ) NOT NULL DEFAULT '".$_SERVER['SERVER_NAME']."' AFTER `direct_access`";
	$sqldescription[47] = "- Adding 'direct_access_text' to settings table...";

	$sql[48] = "ALTER TABLE `".$db['prefix']."spam_log`
			ADD `http_referer` VARCHAR( 255 ) NOT NULL AFTER `user_agent`";
	$sqldescription[48] = "- Adding 'http_referer' to spam_log table...";

	$sql[49] = "ALTER TABLE `".$db['prefix']."settings`
			ADD `search_engines_excluded` INT( 1 ) NOT NULL DEFAULT '1' AFTER `direct_access_text`";
	...
	$sql[52] = "ALTER TABLE `".$db['prefix']."spam_log`
	...
	$sql[53] = "ALTER TABLE `".$db['prefix']."settings`
			ADD `check_against_anti_spam_sites` INT( 1 ) NOT NULL DEFAULT '1' AFTER `search_engines`";
	...
	$sql[62] = "ALTER TABLE `".$db['prefix']."spam`
			ADD `sneaked` INT( 1 ) NOT NULL DEFAULT '0' AFTER `user_agent`";
	$sqldescription[62] = "- Adding 'sneaked' to spam table...";

	$sql[63] = "ALTER TABLE `".$db['prefix']."spam_log`
			ADD `sneaked` INT( 1 ) NOT NULL DEFAULT '0' AFTER `site`";
	$sqldescription[63] = "- Adding 'sneaked' to spam_log table...";

	$sql[64] = "ALTER TABLE `".$db['prefix']."entries`
			CHANGE `ip` `ip` VARCHAR( 255 ) NOT NULL";
	...
	$sql[66] = "ALTER TABLE `".$db['prefix']."user`
			CHANGE `user_ip` `user_ip` VARCHAR( 255 ) NOT NULL";
	$sqldescription[66] = "- Changing maximum length of 'user_ip' in user table to 255...";

	$sql[67] = "ALTER TABLE `".$db['prefix']."settings`
			ADD `ayah_pub_key` VARCHAR( 255 ) NOT NULL AFTER `sfs_api_key`";
	$sqldescription[67] = "- Adding 'ayah_pub_key' to settings table...";

	$sql[68] = "ALTER TABLE `".$db['prefix']."settings`
			ADD `ayah_score_key` VARCHAR( 255 ) NOT NULL AFTER `ayah_pub_key`";
	$sqldescription[68] = "- Adding 'ayah_score_key' to settings table...";
	
	$sql[69] = "ALTER TABLE `".$db['prefix']."settings`
	...
	$sql[71] = "ALTER TABLE `".$db['prefix']."settings`
	...
	$sql[74] = "ALTER TABLE `".$db['prefix']."settings`
	...
	$sql[75] = "ALTER TABLE `".$db['prefix']."settings`
			ADD `banlist_cleanup` TINYINT(1) NOT NULL DEFAULT '1' AFTER `banlist_log`";
	$sqldescription[75] = "- Adding field for automatic banlist cleanup...";

	if(isset($_POST['update_version']) AND $_POST['update_version'] == 1) {
		$sql[76] = "UPDATE `".$db['prefix']."settings` SET `version` = '".MGB_VERSION."'";
		$sqldescription[76] = "- Updating version number...";
	}
?>
